pipeline {
    ...
    environment {
    ACR_NAME = "dannycontainerRegistry1"
    registyUrl = "dannycontainerregistry1.azurecr.io"
    IMAGE_NAME = "nodejswebapp"
    IMAGE_TAG = "v1.0.0"
    registryCredential  = "dannycontainerRegistry1"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/DonBonerunsing/nodejswebapp.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
